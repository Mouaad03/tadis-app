...
export default function PreTradeGate({ profile, todayTrades, onTradeAdded }: Props) {
  ...
  useEffect(() => { if (checklist.length) localStorage.setItem(CHECKLIST_KEY, JSON.stringify(checklist)) }, [checklist])
  useEffect(() => { if (strategies.length) localStorage.setItem(STRATEGIES_KEY, JSON.stringify(strategies)) }, [strategies])
  useEffect(() => {
                  ...
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#0d0d18', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, zIndex: 999, marginTop: 4, overflow: 'hidden' }}>
                    {strategies.map(s => (
                      <div key={s} style={{ display: 'flex', alignItems: 'center', padding: '9px 14px', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer' }}
                        ...
                        <button onClick={() => { setStrategies(p => p.filter(x => x!==s)); if (strategy===s) setStrategy('') }} style={{ background: 'none', border: 'none', color: 'rgba(255,68,102,0.5)', cursor: 'pointer', fontSize: 15 }}>×</button>
                      </div>
                    ))}
                    <div style={{ padding: '8px 10px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: 6 }}>
                      <input value={newStrat} onChange={e => setNewStrat(e.target.value)} onKeyDown={e => { if (e.key==='Enter' && newStrat.trim()) { setStrategies(p => [...p, newStrat.trim()]); setNewStrat('') }}} placeholder={t('addStrategy', lang)} style={{ ...inp, flex: 1, fontSize: 12, padding: '6px 9px' }} />
                      <button onClick={() => { if (newStrat.trim()) { setStrategies(p => [...p, newStrat.trim()]); setNewStrat('') }}} style={{ padding: '6px 11px', background: 'rgba(0,204,119,0.1)', border: '1px solid rgba(0,204,119,0.2)', borderRadius: 7, color: '#00cc77', cursor: 'pointer', fontSize: 13 }}>+</button>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div style={card}>
                ...
                {checklist.map(item => (
                  <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <button onClick={() => setChecklist(p => p.filter(c => c.id!==item.id))} style={{ background: 'none', border: 'none', color: 'rgba(255,68,102,0.4)', cursor: 'pointer', fontSize: 15 }}>×</button>
                  </div>
                ))}
                <div style={{ marginTop: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 7 }}>
                    <input value={newCond} onChange={e => setNewCond(e.target.value)} onKeyDown={e => { if (e.key==='Enter' && newCond.trim()) { setChecklist(p => [...p, {id: Date.now().toString(), text: newCond.trim(), tag: newCondTag, checked: false}]); setNewCond('') }}} placeholder={t('addCondition', lang)} style={{ ...inp, flex: 1, fontSize: 12 }} />
                    <button onClick={() => { if (newCond.trim()) { setChecklist(p => [...p, {id: Date.now().toString(), text: newCond.trim(), tag: newCondTag, checked: false}]); setNewCond('') }}} style={{ padding: '7px 12px', background: 'rgba(0,204,119,0.1)', border: '1px solid rgba(0,204,119,0.2)', borderRadius: 8, color: '#00cc77', cursor: 'pointer', fontSize: 14 }}>+</button>
                  </div>
                </div>
              </div>
