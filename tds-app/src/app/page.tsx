...
const LANGS = {
  en: {
    ...
    plans: [
      { name: 'Free', price: '$0', period: '/forever', features: ['Pre-trade gate', 'Lot calculator', 'Trade journal', 'Up to 10 trades/day', '4 languages'], cta: 'Get started', highlight: false },
      { name: 'Pro', price: '$12', period: '/month', features: ['Everything in Free', 'AI trade feedback', 'Weekly AI report', 'Unlimited trades', 'Priority support'], cta: 'Start Pro', highlight: true },
    ],
    faq_title: 'FAQ',
    faqs: [
    ...
    footer: '© 2025 TRADIS — Trading Discipline System',
    login: 'Login',
  },
  fr: {
    ...
    plans: [
      { name: 'Gratuit', price: '0€', period: '/toujours', features: ['Portail pré-trade', 'Calculateur de lot', 'Journal de trading', 'Jusqu\'à 10 trades/jour', '4 langues'], cta: 'Commencer', highlight: false },
      { name: 'Pro', price: '12€', period: '/mois', features: ['Tout le plan Gratuit', 'Feedback IA par trade', 'Rapport IA hebdomadaire', 'Trades illimités', 'Support prioritaire'], cta: 'Passer Pro', highlight: true },
    ],
    faq_title: 'FAQ',
    faqs: [
    ...
    footer: '© 2025 TRADIS — Système de Discipline de Trading',
    login: 'Connexion',
  },
  ar: {
    ...
    plans: [
      { name: 'مجاني', price: '$0', period: '/للأبد', features: ['بوابة ما قبل الصفقة', 'حاسبة اللوت', 'سجل التداول', 'حتى 10 صفقات/يوم', '4 لغات'], cta: 'ابدأ الآن', highlight: false },
      { name: 'برو', price: '$12', period: '/شهر', features: ['كل ما في المجاني', 'ملاحظات AI لكل صفقة', 'تقرير AI أسبوعي', 'صفقات غير محدودة', 'دعم أولوية'], cta: 'اشترك في برو', highlight: true },
    ],
  ...
  es: {
    ...
    plans: [
      { name: 'Gratis', price: '$0', period: '/siempre', features: ['Puerta pre-trade', 'Calculadora de lote', 'Diario de trading', 'Hasta 10 trades/día', '4 idiomas'], cta: 'Empezar', highlight: false },
      { name: 'Pro', price: '$12', period: '/mes', features: ['Todo lo gratuito', 'Feedback IA por trade', 'Informe IA semanal', 'Trades ilimitados', 'Soporte prioritario'], cta: 'Ir Pro', highlight: true },
    ],
    ...
    footer: '© 2025 TRADIS — Sistema de Disciplina de Trading',
...
export default function LandingPage() {
  ...
  return (
    <div style={{ background: '#08080f', color: '#e8e8f0', fontFamily: "'Syne', sans-serif", minHeight: '100vh', direction: isRTL ? 'rtl' : 'ltr', overflowX: 'hidden' }}>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .grid-bg { background-image: linear-gradient(#ffffff08 1px, transparent 1px), linear-gradient(90deg, #ffffff08 1px, transparent 1px); background-size: 40px 40px; }
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 100, background: '#000000', borderBottom: '1px solid #1a1a28', padding: '0 24px' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '64px' }}>
          <div className="logo-wrap" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            <img src="/logo.jpg" alt="TRADIS" style={{ height: '108px', width: 'auto' }} onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }} />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
            {c.nav.map(n => <a key={n} className="nav-link" href={`#${n.toLowerCase().replace(/\s/g,'-')}`}>{n}</a>)}
            <div style={{ display: 'flex', gap: '6px' }}>
              {(['en','fr','ar','es'] as LangKey[]).map(l => (
                <button key={l} onClick={() => changeLang(l)} style={{ padding: '4px 8px', background: lang === l ? '#00ff8822' : 'none', border: `1px solid ${lang === l ? '#00ff88' : '#1e1e30'}`, borderRadius: '6px', color: lang === l ? '#00ff88' : '#555570', fontSize: '11px', cursor: 'pointer', fontFamily: 'JetBrains Mono' }}>
                  {l.toUpperCase()}
                </button>
              ))}
            </div>
            <button onClick={() => router.push('/auth')} style={{ padding: '8px 18px', background: '#00ff88', border: 'none', borderRadius: '8px', color: '#0a0a0f', fontFamily: 'Syne', fontWeight: 700, fontSize: '13px', cursor: 'pointer' }}>{c.login}</button>
          </div>
        </div>
      </nav>
      ...
      <section id="features" style={{ padding: '100px 24px', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
          {c.features.map((f, i) => (
            <div key={i} className="card-hover" style={{ background: '#0d0d18', border: '1px solid #1a1a28', borderRadius: '16px', padding: '28px', animationDelay: `${i * .1}s` }}>
      ...
      {/* HOW IT WORKS */}
      <section id="how" style={{ padding: '100px 24px', background: '#0a0a12' }}>
      ...
      <section id="pricing" style={{ padding: '100px 24px', maxWidth: '800px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
          {c.plans.map((plan, i) => (
            <div key={i} style={{ background: plan.highlight ? '#0d1a0d' : '#0d0d18', border: `2px solid ${plan.highlight ? '#00ff88' : '#1a1a28'}`, borderRadius: '20px', padding: '36px', position: 'relative' }}>
              {plan.highlight && <div style={{ position: 'absolute', top: '-14px', left: '50%', transform: 'translateX(-50%)', background: '#00ff88', color: '#0a0a0f', fontSize: '11px', fontWeight: 800, padding: '4px 14px', borderRadius: '20px', letterSpacing: '1px', whiteSpace: 'nowrap' }}>MOST POPULAR</div>}
      ...
      {/* FAQ */}
      <section style={{ padding: '80px 24px', background: '#0a0a12' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          <h2 style={{ fontSize: '32px', fontWeight: 800, textAlign: 'center', marginBottom: '48px' }}>{c.faq_title}</h2>
          {c.faqs.map((faq, i) => (
            <div key={i} style={{ borderBottom: '1px solid #1a1a28', padding: '24px 0' }}>
              <div style={{ fontSize: '16px', fontWeight: 700, marginBottom: '10px', color: '#e8e8f0' }}>{faq.q}</div>
              <div style={{ fontSize: '14px', color: '#666680', lineHeight: 1.6 }}>{faq.a}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* FOOTER */}
      <footer style={{ borderTop: '1px solid #1a1a28', padding: '24px', textAlign: 'center', fontSize: '13px', color: '#555570' }}>
        {c.footer}
      </footer>
    </div>
  )
